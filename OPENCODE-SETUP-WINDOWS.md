...
- **Prompt marker (`PS>`)** — commands sometimes start with a marker that says where to paste them: `PS>` for PowerShell on Windows, `$` for Terminal on a Mac. You never type the marker itself — only the command after it. The setup page's copy buttons leave the marker off automatically.
